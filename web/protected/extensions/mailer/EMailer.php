...
class EMailer
{
   ...
   private $_myMailer;
	 ...
	 * Send email to single user.
    	*/
	public function CSend($subject, $email){
		$this->From = Yii::app()->params['email'];
		$this->AddReplyTo(Yii::app()->params['email']);
	...
	/**
	* Send email to a array of users.
	* */
	public function GPSend($subject, $users){
		$this->From = Yii::app()->params['email'];
		$this->AddReplyTo(Yii::app()->params['email']);
		$this->AddAddress(Yii::app()->params['email']);
		$this->FromName = Yii::app()->name;
		$this->IsHTML(true);
		$this->CharSet = 'UTF-8';
		$this->Subject = $subject;
